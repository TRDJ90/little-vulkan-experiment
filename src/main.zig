...
const VulkanContext = @import("vulkan/vulkan_context.zig");
...
pub fn main() !void {
    ...
    std.log.debug("Using device: {s}", .{vc.deviceName()});

    var swapchain = try Swapchain.init(&vc, allocator, extent);
    defer swapchain.deinit();

    const pipeline_layout = try vc.dev.createPipelineLayout(&.{
        .flags = .{},
        .set_layout_count = 0,
    ...
    defer vc.dev.destroyPipelineLayout(pipeline_layout, null);

    const render_pass = try createRenderPass(&vc, swapchain);
    defer vc.dev.destroyRenderPass(render_pass, null);

    const pipeline = try createPipeline(&vc, pipeline_layout, render_pass);
    defer vc.dev.destroyPipeline(pipeline, null);
    ...
    const pool = try vc.dev.createCommandPool(&.{
        .queue_family_index = vc.graphics_queue.family,
    }, null);
    defer vc.dev.destroyCommandPool(pool, null);

    const buffer = try vc.dev.createBuffer(&.{
        .size = @sizeOf(@TypeOf(vertices)),
        .usage = .{ .transfer_dst_bit = true, .vertex_buffer_bit = true },
        .sharing_mode = .exclusive,
    }, null);
    defer vc.dev.destroyBuffer(buffer, null);
    const mem_reqs = vc.dev.getBufferMemoryRequirements(buffer);
    const memory = try vc.allocate(mem_reqs, .{ .device_local_bit = true });
    defer vc.dev.freeMemory(memory, null);
    try vc.dev.bindBufferMemory(buffer, memory, 0);
...
fn createCommandBuffers(
    ...
    try rc.dev.allocateCommandBuffers(&.{
        .command_pool = pool,
        .level = .primary,
        .command_buffer_count = @intCast(cmdbufs.len),
    }, cmdbufs.ptr);
    errdefer rc.dev.freeCommandBuffers(pool, @intCast(cmdbufs.len), cmdbufs.ptr);
    ...
    for (cmdbufs, framebuffers) |cmdbuf, framebuffer| {
        try rc.dev.beginCommandBuffer(cmdbuf, &.{});

        rc.dev.cmdSetViewport(cmdbuf, 0, 1, @ptrCast(&viewport));
        rc.dev.cmdSetScissor(cmdbuf, 0, 1, @ptrCast(&scissor));
        ...
        rc.dev.cmdBeginRenderPass(cmdbuf, &.{
        ...
        rc.dev.cmdBindPipeline(cmdbuf, .graphics, pipeline);
        const offset = [_]vk.DeviceSize{0};
        rc.dev.cmdBindVertexBuffers(cmdbuf, 0, 1, @ptrCast(&buffer), &offset);
        rc.dev.cmdDraw(cmdbuf, vertices.len, 1, 0, 0);

        rc.dev.cmdEndRenderPass(cmdbuf);
        try rc.dev.endCommandBuffer(cmdbuf);
    }
...
fn destroyCommandBuffers(vc: *const VulkanContext, pool: vk.CommandPool, allocator: Allocator, cmdbufs: []vk.CommandBuffer) void {
    vc.dev.freeCommandBuffers(pool, @truncate(cmdbufs.len), cmdbufs.ptr);
...
fn createFramebuffers(vc: *const VulkanContext, allocator: Allocator, render_pass: vk.RenderPass, swapchain: Swapchain) ![]vk.Framebuffer {
    ...
    var i: usize = 0;
    errdefer for (framebuffers[0..i]) |fb| vc.dev.destroyFramebuffer(fb, null);

    for (framebuffers) |*fb| {
        fb.* = try vc.dev.createFramebuffer(&.{
            .render_pass = render_pass,
            .attachment_count = 1,
...
fn destroyFramebuffers(vc: *const VulkanContext, allocator: Allocator, framebuffers: []const vk.Framebuffer) void {
    for (framebuffers) |fb| vc.dev.destroyFramebuffer(fb, null);
...
fn createRenderPass(vc: *const VulkanContext, swapchain: Swapchain) !vk.RenderPass {
    ...
    return try vc.dev.createRenderPass(&.{
        .attachment_count = 1,
...
fn createPipeline(
    ...
    const vert = try vc.dev.createShaderModule(&.{
        .code_size = vert_spv.len,
        .p_code = @ptrCast(&vert_spv),
    }, null);
    defer vc.dev.destroyShaderModule(vert, null);

    const frag = try vc.dev.createShaderModule(&.{
        .code_size = frag_spv.len,
        .p_code = @ptrCast(&frag_spv),
    }, null);
    defer vc.dev.destroyShaderModule(frag, null);
    ...
    _ = try vc.dev.createGraphicsPipelines(
        .null_handle,
        1,
        @ptrCast(&gpci),
        null,
